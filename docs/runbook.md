...
- **Collectors** (headless) crawl configured sites and persist browser state under `sessions/{profileId}`.
...
3. **Scale-Up**
   ```bash
   node src/controller.js start --collectors=20 --visible=6
   ```
   - Increase counts gradually.
   - Monitor CPU (target < 85%), RAM (target < 75%), and disk I/O.
...
- **High CPU or Memory Usage**
  ...
  - Enable resource blocking (`blockResources`) for media-heavy sites.
...
- **Session Not Persisting**
  ...
  - Verify permissions to write under `sessions/`.
...
- **Anti-Bot Detection Triggers**
  ...
  - Add random per-run delays (`humanize.randomDelay`).
